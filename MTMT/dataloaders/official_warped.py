...
def make_union_dataset(root, edge=False):
    ...
    if edge:
        ...
        for split_set in ['train']:
            for img_name in (root_dir / split_set / 'input').iterdir():
                # if split_set in ['train', 'val']:
                if split_set in ['train']:
                    data_list.append((img_name, root_dir / split_set / 'mask_SASMA' / img_name.name, root_dir / split_set / 'mask_SASMA_edge' / img_name.name))
                else:
                    data_list.append((img_name, -1, -1))
    else:
        for split_set in ['train', 'val', 'test']:
            for img_name in (root_dir / split_set / 'input').iterdir():
                if split_set in ['train', 'val']:
                    data_list.append((img_name, root_dir / split_set / 'mask_SASMA' / img_name.name))
                else:
                    data_list.append((img_name, -1))

    return data_list

def make_labeled_dataset(root, edge=False):
    img_list = [os.path.splitext(f)[0] for f in os.listdir(os.path.join(root, 'input')) if f.endswith('.png')]
    label_list = [os.path.splitext(f)[0] for f in os.listdir(os.path.join(root, 'mask_v')) if f.endswith('.png')]
    data_list = []
    if edge:
        # for img_name in img_list:
        for img_name in label_list:
            data_list.append((os.path.join(root, 'input', img_name + '.png'),
                              os.path.join(root, 'mask_SASMA', img_name + '.png'),
                              os.path.join(root, 'mask_SASMA_edge', img_name + '.png')))
    else:
        # for img_name in img_list:
        for img_name in label_list:
            data_list.append((os.path.join(root, 'input', img_name + '.png'),
                                  os.path.join(root, 'mask_v', img_name + '.png')))
    return data_list


class OfficialWarped(data.Dataset):
