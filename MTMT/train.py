...
from dataloaders.official_warped import OfficialWarped
...
parser.add_argument('--root_path', type=str, default='path of data', help='Name of Experiment')
# parser.add_argument('--root_path', type=str, default='datasets/SBU_train', help='Name of Experiment')
...
parser.add_argument('--log_name', type=str, default='mtmt', help='Name of Experiment')
parser.add_argument('--pretrain_weights',type=str, default='weights/iter_10000.pth', help='path of pretrained_weights')
### costs
...
train_data_path = f"{args.root_path}/train"
...
snapshot_path = f'log/{args.log_name}'
tmp_path = f'{snapshot_path}/tmp_see'
...
if __name__ == "__main__":
    ...
    ema_model.load_state_dict(torch.load(args.pretrain_weights))
    ...
    # db_train = SBU(root=train_data_path, joint_transform=joint_transform, transform=img_transform, target_transform=target_transform, mod='labeled', edge=True)
    db_train = OfficialWarped(root=train_data_path, joint_transform=joint_transform, transform=img_transform, target_transform=target_transform, mod='union', edge=True)
    ...
    trainloader = DataLoader(db_train, num_workers=4, worker_init_fn=worker_init_fn)
